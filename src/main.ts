...
import { getMqttConnector, isString, showBloodSugar, showDifference, showElapsedTime, sleep } from './utils.js';
import { BGConnector } from './connector/BGConnector.js';

configDotenv({});

const pageSleeper = (): Promise<void> => sleep(parseInt(process.env.DBIXEL_PAGE_TIMER as string) || 5000);
(async function(): Promise<void> {
  ...
  if (!isString(MQTT_USERNAME) || !isString(MQTT_PASSWORD) || !isString(MQTT_URL) || !isString(MQTT_BASE_TOPIC) || !isString(MQTT_CONNECTION_TIMEOUT) || !isString(MQTT_RECONNECT_PERIOD)) {
